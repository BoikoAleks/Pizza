  ...
  useEffect(() => {
    if (isOpen && productToEdit) {
      form.reset({
        ...
        items: productToEdit.items.length
          ? productToEdit.items.map((item) => ({
              id: item.id,
              price: item.price,
  ...
  const onSubmit = (data: ProductFormValues) => {
    startTransition(async () => {
      try {
        ...
        toast.success(
          `Продукт успішно ${productToEdit ? "оновлено" : "створено"}!`
        );
        handleClose();
      } catch (error: unknown) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <FormItem className="flex-1">
                          <FormControl>
                            <Input
                              {...field}
                              type="number"
                              placeholder="Тип бортика (напр., 1)"
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
            ...
            <FormField<ProductFormValues>
              ...
              render={() => (
                <FormItem>
                  <FormLabel>Доступні інгредієнти</FormLabel>
                            ...
                            <FormControl>
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  ...
                                  return checked
                                    ? field.onChange([...currentValue, ing.id])
                                    : field.onChange(
                                        currentValue.filter(
                                          (id) => id !== ing.id
                                        )

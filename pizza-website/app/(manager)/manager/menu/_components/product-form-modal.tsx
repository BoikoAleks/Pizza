...
export const ProductFormModal = ({ isOpen, categories, productToEdit, ingredients }: Props) => {
  ...
  const form = useForm<ProductFormValues>({
    // zodResolver has a mismatched generic inference in this setup; cast to any to satisfy types
  ...
  items: productToEdit.items.length ? productToEdit.items.map(item => ({ id: item.id, price: item.price, size: item.size ?? undefined, pizzaType: item.pizzaType ?? undefined })) : [{ price: 0 }],
  ...
  const handleClose = () => router.push('/manager/menu?tab=products');
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4 mt-2">
                ...
                <Button type="button" variant="outline" onClick={() => append({ price: 0, size: undefined, pizzaType: undefined })}>
                  <PlusCircle size={16} className="mr-2" /> Додати варіант
                </Button>
              </div>
            </div>
            <FormField<ProductFormValues> name="ingredientIds" control={form.control} render={() => (
